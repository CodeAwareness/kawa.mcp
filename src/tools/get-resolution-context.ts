 ...
 *
 * Two-call protocol:
 *   1. DETECT (cheap, runs every PreToolUse) — `pre_edit_decision_check` returns
 *      `collisions[]` when a live peer (HAI) is editing the same lines.
 *   2. RESOLVE (expensive, opt-in) — only when `collisions[]` is non-empty, call
 *      THIS tool. Muninn decrypts the peer's overlapping diff (it holds the repo
 *      key; the API only stores ciphertext) and returns it + the file's recorded
 *      decisions + a guardrail policy, so you can adapt BEFORE writing.
 *
...
export const getResolutionContextSchema = z.object({
  ...
  peerUid: z
    .string()
    .describe('The peer HAI whose live edits overlap — the `uid` of a collision from pre_edit_decision_check.'),
  filePath: z
    .string()
...
Call this ONLY when pre_edit_decision_check returned a non-empty collisions[] (a live teammate or AI agent is editing the same lines). Pass that collision's uid as peerUid and its overlapping ranges. You get back:
